function T = unperturbed_disc_fvm(D, nodes, elements, boundary_nodes)
%unperturbed_disc_fvm: Solves the PDE del^2 T = -1/D on a disc of radius
%R.  D is the diffusivity, nodes gives the nodes from the relevant mesh,
%similarly for the elements, and boundary_nodes gives the indices in nodes
%corresponding to the boundary of the mesh.
no_elements = size(elements, 1); % number of elements
no_nodes = size(nodes, 1); % number of nodes

%% Geometric Properties

% Specify value of boundary nodes
...
for i = 1:no_nodes
    if variable(i) == 0 % [Eq. 2] This part enforces the Dirichlet condition 
        A(i, i) = 1;
