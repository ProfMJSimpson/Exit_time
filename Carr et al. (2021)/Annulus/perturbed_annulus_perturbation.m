function T = perturbed_annulus_perturbation(R_inner, R_outer, g_inner, g_outer, ...
    D, perturb_degree, N, epsilon, bc_type, backup_inner, backup_outer, backup_D)
...
%perturbation series is n. bc_type is 'absorb-absorb' if inner and outer boundaries are absorbing,
%'reflect-absorb' if inner boundary is reflecting, outer boundary is absorbing, 'absorb-reflect' 
% if inner boundary is absorbing, outer boundary is reflecting.
syms r; assume(r > 0);
syms theta;
...
elseif strcmpi(bc_type, 'absorb-reflect')
    T(1) = (R_inner^2 - r.^2 + 2*R_outer^2 * log(r/R_inner))/(4*D);
    syms dg(theta)
    dg(theta) = diff(g_outer, theta);
    if perturb_degree >= 1
        Hk_inner(theta) = -R_inner*g_inner(theta).*subs(diff(T(1), r), r, R_inner);
        Hk_outer(theta) = -R_outer*g_outer(theta).*subs(diff(T(1), r, 2), r, R_outer) - 2*g_outer(theta) .* subs(diff(T(1), r), r, R_outer);
        C01 = 1/(2*D*sym(pi))*intfix((R_inner^2-R_outer^2)*g_inner(theta)-2*R_outer^2*log(R_inner)*g_outer(theta), theta, 0, 2*sym(pi));
        D01 = R_outer^2/(D*sym(pi))*intfix(g_outer(theta),theta,0,2*sym(pi));
        Vn = R_outer^(-n-1)*R_inner^n+R_outer^(n-1)*R_inner^(-n);
        An1 = 1/(2*D*sym(pi)*n*Vn)*intfix((n*R_outer^(-n-1)*(R_inner^2-R_outer^2)*g_inner(theta)+2*R_inner^(-n)*R_outer*g_outer(theta))*sin(n*theta),theta,0,2*sym(pi));
        Bn1 = 1/(2*D*sym(pi)*n*Vn)*intfix((n*R_outer^(n-1)*(R_inner^2-R_outer^2)*g_inner(theta)-2*R_inner^(n)*R_outer*g_outer(theta))*sin(n*theta),theta,0,2*sym(pi));
        Cn1 = 1/(2*D*sym(pi)*n*Vn)*intfix((n*R_outer^(-n-1)*(R_inner^2-R_outer^2)*g_inner(theta)+2*R_inner^(-n)*R_outer*g_outer(theta))*cos(n*theta),theta,0,2*sym(pi));
        Dn1 = 1/(2*D*sym(pi)*n*Vn)*intfix((n*R_outer^(n-1)*(R_inner^2-R_outer^2)*g_inner(theta)-2*R_inner^(n)*R_outer*g_outer(theta))*cos(n*theta),theta,0,2*sym(pi));
        [~, dAn1] = numden(An1);
        [~, dBn1] = numden(Bn1);
        [~, dCn1] = numden(Cn1);
        [~, dDn1] = numden(Dn1);
        ...
        if ~isempty(divideZeroIdx)
            for nn = divideZeroIdx
                Ann1 = 1/(2*D*sym(pi)*nn*subs(Vn, n, nn))*intfix((nn*R_outer^(-nn-1)*(R_inner^2-R_outer^2)*g_inner(theta)+2*R_inner^(-nn)*R_outer*g_outer(theta))*sin(nn*theta),theta,0,2*sym(pi));
                Bnn1 = 1/(2*D*sym(pi)*nn*subs(Vn, n, nn))*intfix((nn*R_outer^(nn-1)*(R_inner^2-R_outer^2)*g_inner(theta)-2*R_inner^(nn)*R_outer*g_outer(theta))*sin(nn*theta),theta,0,2*sym(pi));
                Cnn1 = 1/(2*D*sym(pi)*nn*subs(Vn, n, nn))*intfix((nn*R_outer^(-nn-1)*(R_inner^2-R_outer^2)*g_inner(theta)+2*R_inner^(-nn)*R_outer*g_outer(theta))*cos(nn*theta),theta,0,2*sym(pi));
                Dnn1 = 1/(2*D*sym(pi)*nn*subs(Vn, n, nn))*intfix((nn*R_outer^(nn-1)*(R_inner^2-R_outer^2)*g_inner(theta)-2*R_inner^(nn)*R_outer*g_outer(theta))*cos(nn*theta),theta,0,2*sym(pi));
                T(2) = T(2) + (Cnn1*r^nn + Dnn1*r^(-nn))*cos(nn*theta) + (Ann1*r^nn + Bnn1*r^(-nn))*sin(nn*theta);
            end
        end
        for nn = setdiff(1:N, divideZeroIdx)
        ...
        if perturb_degree >= 2
            for k = 2:perturb_degree
                ...
                Hk_outer(theta) = -R_outer^k*g_outer(theta).^k/factorial(k)*subs(diff(T(1), r, k+1), r, R_outer);
                Hk_outer(theta) = Hk_outer(theta) - R_outer^(k-1)*g_outer(theta).^(k-1)/factorial(k-1)*subs(diff(T(2), r, k), r, R_outer);
                Hk_outer(theta) = Hk_outer(theta) - 2*R_outer^(k-1)*g_outer(theta).^k/factorial(k-1)*subs(diff(T(1), r, k), r, R_outer);
                for j=1:k-2
                    Hk_outer(theta) = Hk_outer(theta) - R_outer^j*g_outer(theta).^j/factorial(j)*subs(diff(T(k-j+1), r, j+1), r, R_outer);
                end
                for j=0:k-2
                    Hk_outer(theta) = Hk_outer(theta) + R_outer^(j-1)*g_outer(theta).^j.*dg(theta)/factorial(j)*subs(diff(diff(T(k-j), theta), r, j), r, R_outer);
                    Hk_outer(theta) = Hk_outer(theta) - 2*R_outer^j*g_outer(theta).^(j+1)/factorial(j)*subs(diff(T(k-j), r, j+1), r, R_outer);
                    Hk_outer(theta) = Hk_outer(theta) - R_outer^j*g_outer(theta).^(j+2)/factorial(j)*subs(diff(T(k-j-1), r, j+1), r, R_outer);
                end
                C0k = 1/sym(pi) * intfix(Hk_inner(theta) - R_outer*log(R_inner)*Hk_outer(theta), theta, 0, 2*sym(pi));
                D0k = 1/sym(pi) * intfix(R_outer*Hk_outer(theta), theta, 0, 2*sym(pi));
                Vn = R_outer^(-n-1)*R_inner^n + R_outer^(n-1)*R_inner^(-n);
                Ank = 1/(n*sym(pi)*Vn) * intfix((n*R_outer^(-n-1)*Hk_inner(theta) + R_inner^(-n)*Hk_outer(theta))*sin(n*theta), theta, 0, 2*sym(pi));
                Bnk = 1/(n*sym(pi)*Vn) * intfix((n*R_outer^(n-1)*Hk_inner(theta) - R_inner^(-n)*Hk_outer(theta))*sin(n*theta), theta, 0, 2*sym(pi));
                Cnk = 1/(n*sym(pi)*Vn) * intfix((n*R_outer^(-n-1)*Hk_inner(theta) + R_inner^(-n)*Hk_outer(theta))*cos(n*theta), theta, 0, 2*sym(pi));
                Dnk = 1/(n*sym(pi)*Vn) * intfix((n*R_outer^(n-1)*Hk_inner(theta) - R_inner^(-n)*Hk_outer(theta))*cos(n*theta), theta, 0, 2*sym(pi));
                [~, dAnk] = numden(Ank);
                [~, dBnk] = numden(Bnk);
                [~, dCnk] = numden(Cnk);
                [~, dDnk] = numden(Dnk);
                ...
                if ~isempty(divideZeroIdx)
                    for nn = divideZeroIdx
                        Annk = 1/(nn*sym(pi)*subs(Vn, n,nn)) * intfix((nn*R_outer^(-nn-1)*Hk_inner(theta) + R_inner^(-nn)*Hk_outer(theta))*sin(nn*theta), theta, 0, 2*sym(pi));
                        Bnnk = 1/(nn*sym(pi)*subs(Vn,n, nn)) * intfix((nn*R_outer^(nn-1)*Hk_inner(theta) - R_inner^(-nn)*Hk_outer(theta))*sin(nn*theta), theta, 0, 2*sym(pi));
                        Cnnk = 1/(nn*sym(pi)*subs(Vn,n, nn)) * intfix((nn*R_outer^(-nn-1)*Hk_inner(theta) + R_inner^(-nn)*Hk_outer(theta))*cos(nn*theta), theta, 0, 2*sym(pi));
                        Dnnk = 1/(nn*sym(pi)*subs(Vn,n, nn)) * intfix((nn*R_outer^(nn-1)*Hk_inner(theta) - R_inner^(-nn)*Hk_outer(theta))*cos(nn*theta), theta, 0, 2*sym(pi));
                        T(k+1) = T(k+1) + (Cnnk*r^nn + Dnnk*r^(-nn))*cos(nn*theta) + (Annk*r^nn + Bnnk*r^(-nn))*sin(nn*theta);
                    end
                end
                for nn = setdiff(1:N, divideZeroIdx)
